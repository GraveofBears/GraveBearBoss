...
namespace PieceManager
{
    ...
    public class BuildPiece
    {
        ...
        internal static void Patch_FejdStartup()
        {
            if (ConfigurationEnabled)
            {
                foreach (BuildPiece piece in registeredPieces)
                {
                    ...
                    {
                        ConfigurationManagerAttributes attributes = new() { CustomDrawer = drawConfigTable, Order = --order };
        ...
        private static void drawConfigTable(ConfigEntryBase cfg)
        {
            ...
            foreach (Requirement req in new SerializedRequirements((string)cfg.BoxedValue).Reqs)
            {
                ...
                if (GUILayout.Toggle(req.recover, "Recover", new GUIStyle(GUI.skin.toggle) { fixedWidth = 67 }) != req.recover)
                {
                    recover = !recover;
                    wasUpdated = true;
                }
        ...
        private class SerializedRequirements
        {
            ...
            public static Piece.Requirement[] toPieceReqs(SerializedRequirements craft)
            {
                ...
                Dictionary<string, Piece.Requirement?> resources = craft.Reqs.Where(r => r.itemName != "").ToDictionary(r => r.itemName, r => ResItem(r) is { } item ? new Piece.Requirement { m_amount = r.amount, m_resItem = item, m_recover = r.recover } : null);
    ...
    public static class PiecePrefabManager
    {
        static PiecePrefabManager()
        {
            ...
            harmony.Patch(AccessTools.DeclaredMethod(typeof(ZNetScene), nameof(ZNetScene.Awake)), postfix: new HarmonyMethod(AccessTools.DeclaredMethod(typeof(PiecePrefabManager), nameof(Patch_ZNetSceneAwakePost))));
        ...
        {
            foreach (GameObject prefab in piecePrefabs.Concat(ZnetOnlyPrefabs))
            {
        ...
        {
            if (__instance.GetPrefab("Hammer")?.GetComponent<ItemDrop>().m_itemData.m_shared.m_buildPieces is not { } hammerPieces)
            {
                return;
            }
